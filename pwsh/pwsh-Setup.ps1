...
if (Test-Path $machineSetup -PathType Container)
{
    ...
    #Create symbolic link of keys file to powershell profile folder
    New-Item -ItemType SymbolicLink -Value ([IO.Path]::Combine("$machineSetup","pwsh","keys.ps1")) -Path $PROFILE_FOLDER -Name "keys.ps1" -Force > $null
}
